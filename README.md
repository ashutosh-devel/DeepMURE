...
This repository contains the official PyTorch implementation for the paper: **"DeepMURE: Deep Self-Supervised Despeckling via Monte Carlo Multiplicative Unbiased Risk Estimation"** (Submitted to IEEE Transactions on Image Processing, 2026).
...
To check the estimation performance of DeepMURE w.r.t oracle use estimation_demo.py. You can configure the dataset paths, the equivalent number of looks (L), and model hyperparameters within the script. Graphs are saved in the same folder, 
### Basic run
```bash
...
- Shallow Feature Extraction: Initial 3×3 convolutions extract foundational features.
...
@article{gupta2026deepmure,
  ...
  journal={IEEE Transactions on Image Processing (submission)},
  year={2026}
}
```